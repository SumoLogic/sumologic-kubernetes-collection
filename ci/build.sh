...
if [ -n "$GITHUB_TOKEN" ] && [ "$TRAVIS_EVENT_TYPE" == "pull_request" ]; then
  ...
  prometheus_remote_write=`yq r deploy/helm/prometheus-overrides.yaml prometheus.prometheusSpec.remoteWrite -j`
  echo "// This file is autogenerated" > deploy/kubernetes/kube-prometheus-sumo-logic-mixin.libsonnet
  sed 's#\[\/\*REMOTE_WRITE\*\/\]#'$prometheus_remote_write'#' ci/jsonnet-mixin.tmpl | sed 's#"http://collection-sumologic.sumologic.svc.cluster.local:9888\/# $._config.sumologicCollectorSvc+"#g' > deploy/kubernetes/kube-prometheus-sumo-logic-mixin.libsonnet
