...
# Exclude SC2154 which complains about undeclared but used variables that we get from our CI environment
# e.g. ${TRAVIS_EVENT_TYPE} and ${TRAVIS_PULL_REQUEST_BRANCH}
shellcheck --enable all --exclude SC2154 ci/build.sh
...
if [ -n "${GITHUB_TOKEN}" ] && [ "${TRAVIS_EVENT_TYPE}" == "pull_request" ]; then
  ...
  # Not sure why shellcheck complains about not used 'with_files'.
  # Probably cannot see it used in eval since when removed the warning disappears.
  ...
  # We want to use single quoutes to prevent expansion of $(CHART) and $(NAMESPACE) in sed
