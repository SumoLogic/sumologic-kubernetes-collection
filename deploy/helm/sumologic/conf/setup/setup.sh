...
function delete_hosted_collector() {
    ...
    if [[ -n "${COLLECTOR_ID}" ]]; then
        ...
        if curl -sf -XDELETE \
            -u "${SUMOLOGIC_ACCESSID}:${SUMOLOGIC_ACCESSKEY}" \
            "${SUMOLOGIC_BASE_URL}v1/collectors/${COLLECTOR_ID}"; then
            echo "Hosted collector deleted."
        else
            echo "Failed to delete hosted collector '${SUMOLOGIC_COLLECTOR_NAME}' (${COLLECTOR_ID})."
            return 1
        fi
    else
...
function import_installation_token() {
    ...
    if ! jq -e '.data' <<< "${RESPONSE}" > /dev/null 2>&1; then
        echo "WARNING: Token API response does not contain .data — skipping token import. Response: ${RESPONSE}"
        return 0
    fi
