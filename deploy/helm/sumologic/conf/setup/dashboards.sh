...
function load_dashboards_folder_id() {
  ...
  K8S_FOLDER_ID="$( echo "${ADMIN_FOLDER_CHILDREN}" | \
...
if [[ -z "${K8S_FOLDER_ID}" ]]; then
  APP_INSTALL_JOB_RESPONSE="$(curl -XPOST -s \
         ...
         -d "{\"name\":\"${K8S_FOLDER_NAME}\",\"destinationFolderId\":\"${ADMIN_FOLDER_ID}\",\"description\":\"Kubernetes dashboards provided by Sumo Logic.\"}" \
  ...
  if [ "${APP_INSTALL_JOB_STATUS}" != "Success" ]; then
    ...
    PERMS_RESPONSE="$(curl -XPUT -s \
            ...
            "${SUMOLOGIC_BASE_URL}"v2/content/"${K8S_FOLDER_ID}"/permissions/add )"
    readonly PERMS_RESPONSE
