...
```bash
OS=linux ARCH=amd64; curl -L "https://github.com/SumoLogic/sumologic-kubernetes-tools/releases/download/v2.18.0/update-collection-v3-sumo-${OS}_${ARCH}" -o update-collection-v3
...
docker run \
  --rm \
  -v $(pwd):/values \
  -i sumologic/kubernetes-tools:2.18.0 \
