...
1. Create `sumologic-debug` pod
   ...
   spec:
     ...
     - args:
       ...
       name: debug
     serviceAccountName: <service account name used by prometheus or OpenTelemetry Metrics Collector (e.g. collection-kube-prometheus-prometheus, collection-sumologic-metrics)>
   EOF
   ```
...
```yaml
kubectl -n sumologic logs collection-sumologic-metrics-collector-0 | grep 'serviceMonitor/sumologic/collection-sumologic-otelcol-logs/0'
kubectl -n sumologic logs collection-sumologic-metrics-collector-1 | grep 'serviceMonitor/sumologic/collection-sumologic-otelcol-logs/0'
