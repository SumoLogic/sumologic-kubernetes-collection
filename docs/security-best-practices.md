...
For example, to use `0.68.0-sumo-0-fips` image with Helm Chart v3 use the following configuration:

```yaml
metadata:
  image:
    tag: 0.68.0-sumo-0-fips
otellogs:
  image:
    tag: 0.68.0-sumo-0-fips
otelevents:
...
For example, to use `0.68.0-sumo-0-fips` image with Helm Chart v2 use the following configuration:
