...
For example, to use `0.73.0-sumo-1-fips` image with Helm Chart v3 use the following configuration:

```yaml
metadata:
  image:
    tag: 0.73.0-sumo-1-fips
otellogs:
  image:
    tag: 0.73.0-sumo-1-fips
otelevents:
...
For example, to use `0.73.0-sumo-1-fips` image with Helm Chart v2 use the following configuration:
